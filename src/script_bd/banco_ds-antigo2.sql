...
call cadastrar_Usuario('samuel', 'samu', 1);
...
begin
	select Fornecedor.nome_forn as 'nome' from Fornecedor;
end;
$$ delimiter ;
...
-- diminuir a variável após o cancelamento da compra
delimiter $$
create trigger diminuirNum_compra after delete on Compra_Produto for each row
begin
	select cod_comp into @Num_compra from Compra_Produto where ((cod_comp = last_insert_id() -1));
end;
$$ delimiter ;
...
delimiter $$
create procedure comprarProdutos(data_comp date, formpag_comp varchar(100), cod_forn_CP integer)
begin

if ((select cod_forn from fornecedor where cod_forn = cod_forn_CP)is not null) then
	if (formpag_comp = 'Dinhero') or (formpag_comp = 'cartão de crédito') then
		insert into Compra_Produto values (null, data_comp, 0, formpag_comp, cod_forn_CP);
		select @Num_compra as resposta;
	else
		select 'Verifique a forma de pagamento' as Alerta;
	end if;
else
...
call comprarProdutos('2019-09-09','Dinhero', 1);
...
update Produto set quant_prod = quant_prod - old.quant_itenscomp 
...
-- relatório de Produtpo
select
Produto.nome_prod as "Nome Produto",
Produto.valor_Unit_vend_prod as "Valor da Venda",
Produto.valor_Unit_Forn_prod as "Valor de Compra",
Produto.quant_prod as "Quantidade",

#Fazendo a ligação entre tabales(Produto.cod_forn_Prod= Fornecedor.cod_forn)
Fornecedor.nome_forn as "Fornecedor"
from Produto, Fornecedor
where Produto.cod_forn_Prod= Fornecedor.cod_forn;